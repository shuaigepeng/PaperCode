...
def affinitypropagation_plus(uav_list, net_args):
    """
    ...
    model = AffinityPropagation(damping=0.5, max_iter=500, convergence_iter=30, affinity="precomputed")
    sc = model.fit(weight_matrix).labels_
    result = [[] * (max(sc)+1) for _ in range(max(sc)+1)]
...
def split_cluster(cluster_result, n_max, net_args):
    ...
    for uavs_part in cluster_result:
        ...
        if len(uavs_part) > n_max:
            ...
            result = affinitypropagation_plus(uavs_part_tmp, net_args)
            # 删除空簇
...
def affinitypropagation_final(uav_num, n_max):
    ...
    print("affinitypropagation：", all_uav_number_packet)
    return cluster_result, all_uav_number_packet
...
if __name__ == '__main__':
    ...
    # # 每个簇最大的数
    # n_max = 20
    affinitypropagation_final(100, 0)
