...
def agglomerativeclustering_plus(uav_list, k, net_args):
    """
    谱聚类加了个结果收集
    ...
    sc = AgglomerativeClustering(n_clusters=k).fit(H).labels_
...
def split_cluster(cluster_result, n_max, net_args):
    ...
    for uavs_part in cluster_result:
        ...
        if len(uavs_part) > n_max:
            ...
            result = agglomerativeclustering_plus(uavs_part_tmp, int(math.ceil(len(uavs_part)/n_max)), net_args)
            # 删除空簇
        ...
        if not is_has_candidate_head(uavs_part, net_args):
            uavs_part_tmp = uavs_part
            cluster_result.remove(uavs_part)
            result = agglomerativeclustering_plus(uavs_part_tmp, 2, net_args)
            # 删除空簇
            result = delete_empty_line(result)
            cluster_result = combine_list(cluster_result, result)
            return cluster_result
...
def agglomerativeclustering_final(uav_num, n_max):
    ...
    print("agglomerativeclustering：", all_uav_number_packet)
    # print("----------------end-----------------------")
    return cluster_result, all_uav_number_packet
...
if __name__ == '__main__':
    ...
    # 每个簇最大的数
    # n_max = 15
    agglomerativeclustering_final(100, 25)
