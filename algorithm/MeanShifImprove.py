...
def meanshift_plus(uav_list, net_args):
    """
    ...
    bw = sc.estimate_bandwidth(H, n_samples=len(H), quantile=0.1)
    model = sc.MeanShift(bandwidth=bw, bin_seeding=True)
    model.fit(H)  # 完成聚类
    pred_y = model.predict(H)  # 预测点在哪个聚类中
    result = [[] * (max(pred_y)+1) for _ in range(max(pred_y)+1)]
    for index in range(len(uav_list)):
        cluster_id = pred_y[index]
...
def split_cluster(cluster_result, n_max, net_args):
    ...
    for uavs_part in cluster_result:
        ...
        if len(uavs_part) > n_max:
            ...
            result = meanshift_plus(uavs_part_tmp, net_args)
            # 删除空簇
...
def meanshift_final(uav_num, n_max):
    print("meanshift")
    # # 无人机总数
    # uav_num = 100
    # # 每个簇最大的数
    ...
    print("meanshift：", all_uav_number_packet)
    return cluster_result, all_uav_number_packet
