...
from network import set_network_param, generate_uav, get_weight_matrix, get_adj_matrix, cal_SINR, db_to_ratio, get_real_weight_matrix
import numpy as np


def sc_algorithm_sklearn(weight_matrix, k):
    sc = SpectralClustering(n_clusters=k, affinity='precomputed', assign_labels='discretize', random_state=1)
    # sc.set_params([affinity=precomputed])
    sc.fit(weight_matrix)
    return sc.labels_


def spectral_clustering_plus(uav_list, k, net_args):
    """
    谱聚类加了个结果收集
    :param uav_list: 无人机集合
    :return: 分簇结果
    """
    weight_matrix = get_weight_matrix(uav_list, net_args)
    sc = sc_algorithm_sklearn(weight_matrix, k)
...
def split_cluster(cluster_result, n_max, net_args):
    ...
    for uavs_part in cluster_result:
        ...
        if len(uavs_part) > n_max:
            ...
            result = spectral_clustering_plus(uavs_part_tmp, int(math.ceil(len(uavs_part)/n_max)), net_args)
            # 删除空簇
        ...
        if not is_has_candidate_head(uavs_part, net_args):
            uavs_part_tmp = uavs_part
            cluster_result.remove(uavs_part)
            result = spectral_clustering_plus(uavs_part_tmp, 2, net_args)
            # 删除空簇
            result = delete_empty_line(result)
            cluster_result = combine_list(cluster_result, result)
            return cluster_result
    ...
    for cluster in cluster_result:
        ...
        for i in range(len(cluster)):
            if len(cluster[i].neighbor) == len(cluster)-1:
                ...
                if cluster_number_packet > max_cluster_number_packet:
...
def sc_final(uav_num, n_max):
    ...
    print("谱聚类：", all_uav_number_packet)
    # print("----------------end-----------------------")
    return cluster_result, all_uav_number_packet


def sc_final_new(uav_list, n_max, net_args):
    print("谱聚类")
    # 初始化分簇结果
    cluster_result = [uav_list]
    # 更新簇内邻居
